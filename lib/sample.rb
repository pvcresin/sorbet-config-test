...
class Sample
  ...
  # T.letでStringの値をIntegerに変換しようとしている -> inline_type_error_handler
  ...
  # sig 内で String を返すと指定しているが、実際は Integer が返ってくる -> call_validation_error
  sig { params(num: Integer).returns(String) }
  def b(num)
    1
  end

  # sig内のprocで .returns や .void を指定してないので -> sig_builder_error_handler
  sig { params(num: Integer) }
  def c(num)
    1
  end

  # .overrideと書いているが、実際はoverrideしていない -> sig_validation_error_handler
